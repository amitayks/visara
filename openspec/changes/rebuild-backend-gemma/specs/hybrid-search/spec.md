# hybrid-search — Delta Spec (requirement set replaced wholesale)

## REMOVED Requirements

### Requirement: Hybrid search augments lexical results with a semantic ranker
**Reason**: MiniSearch + in-memory MiniLM matrix are deleted; hybrid search is rebuilt over FTS5 + sqlite-vec in SQL.
**Migration**: See ADDED requirements below.

### Requirement: The lexical search API is preserved and hybrid is additive
**Reason**: The MiniSearch lexical API no longer exists.
**Migration**: FTS5 is the lexical arm; see ADDED requirements below.

### Requirement: Hybrid search degrades gracefully to lexical-only
**Reason**: Replaced by the new degradation ladder over FTS5/vec0.
**Migration**: See "Degradation ladder" below.

### Requirement: Hybrid search fulfills the natural-language search promise
**Reason**: Replaced by the EmbeddingGemma-based semantic arm.
**Migration**: See ADDED requirements below and `gemma-embedding-index`.
