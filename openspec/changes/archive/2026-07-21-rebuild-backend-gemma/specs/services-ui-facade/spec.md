# services-ui-facade — Delta Spec (requirement set replaced wholesale)

## REMOVED Requirements

### Requirement: Hydrated hybrid search through searchMedia
**Reason**: Rewritten below against the SQL hybrid path (behavior preserved, internals replaced).
**Migration**: See ADDED "searchMedia returns hydrated rows in fused order".

### Requirement: Complete media removal through one public path
**Reason**: Rewritten below — permanent deletion now flows through the OS-confirmed `MediaIndexer.deleteAssets`, and cleanup is a single-transaction purge.
**Migration**: See ADDED "removeMedia is the single removal path".

### Requirement: Search index lifecycle owned by ensureSearchIndex
**Reason**: There are no side-indexes to load or rebuild; `ensureSearchIndex` is retired.
**Migration**: Search reads live FTS5/vec0 tables (`hybrid-search`); call sites drop the call.

### Requirement: Post-onboarding boot order preserved exactly
**Reason**: Rewritten for the new boot sequence (LibrarySync discovery gate before pipeline).
**Migration**: See ADDED "Boot sequence gates processing on discovery".

### Requirement: Permission denial yields a recoverable denied state
**Reason**: Carried over with the new permission source (`MediaIndexer.requestAccess`).
**Migration**: See ADDED "Permission denial is recoverable in-session".

### Requirement: Orchestrator events feed the processing snapshot through one subscription
**Reason**: Carried over verbatim in behavior; the emitting module is the new Pipeline.
**Migration**: See ADDED "Pipeline events feed the processing snapshot through one subscription".

### Requirement: Live media observer runs from post-onboarding boot to shutdown
**Reason**: Carried over; the observer is now MediaIndexer's poke + LibrarySync delta round.
**Migration**: See ADDED "Live observation runs from post-onboarding boot to shutdown".

### Requirement: Drain settings are boolean-typed, single-owner, and forwarded on change
**Reason**: Carried over; the sink is the new Pipeline's gate config.
**Migration**: See ADDED "Drain settings forwarded to pipeline gates".
