...
export class SearchOrchestrator {
	...
	private async initializeSemanticSearch(): Promise<void> {
		if (this.useOnnxIfAvailable) {
			try {
				...
				console.warn(
					"Semantic search initialization failed, using fallback:",
					error,
				);
				this.useOnnxIfAvailable = false;
			}
	...
	async search(
		...
		if (options.useSemanticSearch !== false && documents.length > 0) {
			try {
				...
				if (this.useOnnxIfAvailable && semanticSearchService.isLoaded) {
					...
					searchVectors =
						await this.generateDocumentEmbeddingsWithOnnx(documents);
				} else {
		...
		const finalDocuments = this.postProcess(
			scoredDocuments,
			parsedQuery,
			options,
		);
	...
	private async fetchDocuments(
		...
		const documentsCollection =
			this.database.collections.get<Document>("documents");
		...
		if (query.temporal) {
			if (query.temporal.type === "count" && query.temporal.count) {
				...
				if (query.temporal.documentType) {
					conditions.push(
						Q.where("document_type", query.temporal.documentType),
					);
				}
			} else {
				// Date range filter
				if (query.temporal.startDate) {
					conditions.push(
						Q.where("date", Q.gte(query.temporal.startDate.getTime())),
					);
				}
				if (query.temporal.endDate) {
					conditions.push(
						Q.where("date", Q.lte(query.temporal.endDate.getTime())),
					);
				}
			}
		}
			...
			switch (query.amount.operator) {
				...
				case "between":
					conditions.push(
						...
						Q.where(
							"total_amount",
							Q.lte(query.amount.maxValue || query.amount.value),
						),
					);
					break;
			}
		...
		if (query.vendor && query.vendor.length > 0) {
			filtered = filtered.filter((doc) => {
				...
				return query.vendor!.some((vendor) => {
					...
					if (
						docVendor.includes(vendorLower) ||
						vendorLower.includes(docVendor)
					) {
						return true;
					}
	...
	private async generateDocumentEmbeddingsWithOnnx(
		...
		for (const doc of documents) {
			if (
				doc.searchVector &&
				doc.searchVector.length === semanticSearchService.outputSize
			) {
		...
		if (docsNeedingEmbeddings.length > 0) {
			const texts = docsNeedingEmbeddings.map((doc) =>
				this.getDocumentText(doc),
			);
			const newEmbeddings =
				await semanticSearchService.generateBatchEmbeddings(texts);
	...
	private applyContext(query: ParsedQuery, context: QueryContext): void {
		...
		if (context.userPreferences) {
			...
			if (
				!query.documentTypes &&
				context.userPreferences.preferredDocumentTypes
			) {
	...
	private getDocumentText(doc: Document): string {
		...
		if (doc.metadata) {
			...
			if (metadata.items) {
				parts.push(
					...metadata.items.map((item: any) => item.description || ""),
				);
			}
		}
