...
export class SimpleSearchService {
  async search(query: string): Promise<SearchResult> {
    ...
    console.log(`[SearchService] Total documents in database: ${allDocuments.length}`);
    ...
    if (parsedQuery.dateRange) {
      console.log('[SearchService] Applying date filter:', parsedQuery.dateRange);
    ...
    if (parsedQuery.searchTerms.length > 0) {
      console.log('[SearchService] Applying text search for terms:', parsedQuery.searchTerms);
      ...
      results = results.filter(doc => {
        ...
        return parsedQuery.searchTerms.some((term: string) => {
          ...
          if (searchableText.includes(termLower)) {
            console.log(`[SearchService] Found exact match for "${term}" in document ${doc.id}`);
            return true;
          }
          ...
          if (doc.vendor) {
            ...
            const phoneticMatch = vendorWords.some(vw => 
              termWords.some(tw => 
                PhoneticMatcher.similarity(vw, tw) > 0.75
              )
            );
            
            if (phoneticMatch) {
              console.log(`[SearchService] Found phonetic match for "${term}" in vendor "${doc.vendor}"`);
              return true;
            }
          }
  ...
  private parseQuery(query: string): any {
    ...
    const commonWords = new Set([
      'show', 'find', 'search', 'get', 'from', 'the', 'a', 'an',
      'for', 'me', 'my', 'all', 'with', 'in', 'on', 'at',
  ...
  generateResponse(result: SearchResult): string {
    ...
    if (documents.length === 0) {
      ...
      response += ". Try different search terms or check if the documents have been scanned.";
      return response;
    }
