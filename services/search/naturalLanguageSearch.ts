...
export class NaturalLanguageSearchService {
  ...
  private extractDateRange(query: string): { start: Date; end: Date } | null {
    ...
    for (const [hebrew, english] of Object.entries(this.hebrewDatePatterns)) {
      if (query.includes(hebrew)) {
        const pattern = this.datePatterns[english as keyof typeof this.datePatterns];
        if (pattern) {
          return pattern();
        }
      }
    ...
    const relativeDaysMatch = query.match(/past (\d+) days?|(\d+) ימים אחרונים/i);
  ...
  private extractVendors(query: string): string[] {
    ...
    const fromMatch = query.match(/(?:from|at|של|מ)\s+([A-Za-z\u0590-\u05FF]+(?:\s+[A-Za-z\u0590-\u05FF]+)?)/gi);
    if (fromMatch) {
  ...
  private isCommonWord(word: string): boolean {
    const commonWords = ['the', 'from', 'at', 'in', 'on', 'and', 'or', 'for', 'with'];
  ...
  private async executeSearch(query: SearchQuery): Promise<Document[]> {
    ...
    if (query.amountRange) {
      documents = documents.filter(doc => {
        ...
        if (query.amountRange!.currency && doc.currency !== query.amountRange!.currency) {
          return false;
        }
        
        if (query.amountRange!.min && doc.totalAmount < query.amountRange!.min) {
          return false;
        }
        
        if (query.amountRange!.max && doc.totalAmount > query.amountRange!.max) {
          return false;
        }
    ...
    if (query.vendors && query.vendors.length > 0) {
      documents = documents.filter(doc => {
        ...
        return query.vendors!.some(queryVendor => {
          ...
          return vendorLower.includes(queryLower) || 
    ...
    if (!query.dateRange && !query.amountRange && !query.vendors && !query.documentTypes) {
