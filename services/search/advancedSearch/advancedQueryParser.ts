...
export class AdvancedQueryParser {
	...
	parse(query: string): ParsedQuery {
		...
		const documentTypes =
			this.entityExtractor.extractDocumentTypesList(normalizedQuery);
		...
		const confidence = this.calculateQueryConfidence(
			entities,
			intents,
			temporal,
		);
	...
	requiresAggregation(query: ParsedQuery): boolean {
		return query.intent.includes("aggregate") || query.intent.includes("count");
	}
}
