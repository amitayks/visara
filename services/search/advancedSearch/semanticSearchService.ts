...
export class SemanticSearchService implements EmbeddingModel {
	...
	async generateEmbedding(text: string): Promise<number[]> {
		...
		try {
			...
			const inputIds = new Tensor("int64", tokens.inputIds, [
				1,
				this.inputSize,
			]);
			const attentionMask = new Tensor("int64", tokens.attentionMask, [
				1,
				this.inputSize,
			]);
			...
			const embeddings =
				output.embeddings || output.pooler_output || output.last_hidden_state;
	...
	calculateRelevanceScore(
		...
		const semanticScore = this.calculateSimilarity(
			queryEmbedding,
			documentEmbedding,
		);
...
class SimpleTokenizer {
	...
	tokenize(
		text: string,
		maxLength: number,
	): { inputIds: bigint[]; attentionMask: bigint[] } {
