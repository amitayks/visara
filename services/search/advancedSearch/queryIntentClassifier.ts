import { QueryIntent, QueryClassification, ExtractedEntity } from './searchTypes';
...
export class QueryIntentClassifier {
    ...
    for (const entity of entities) {
      switch (entity.type) {
        ...
        case 'amount':
          ...
          if (amountText.includes('over') || amountText.includes('under') ||
              amountText.includes('above') || amountText.includes('below')) {
  ...
  private determinePrimaryIntent(query: string, intents: QueryIntent[]): QueryIntent {
  ...
  private isEntityRelevantToIntent(entity: ExtractedEntity, intent: QueryIntent): boolean {
  ...
  combineIntents(intents1: QueryIntent[], intents2: QueryIntent[]): QueryIntent[] {
  ...
  getSortCriteria(query: string): { field: 'date' | 'amount' | 'relevance'; order: 'asc' | 'desc' } {
    ...
    if (normalizedQuery.includes('expensive') || normalizedQuery.includes('highest') ||
        normalizedQuery.includes('most') || normalizedQuery.includes('יקר')) {
      return { field: 'amount', order: 'desc' };
    }

    if (normalizedQuery.includes('cheapest') || normalizedQuery.includes('lowest') ||
        normalizedQuery.includes('least') || normalizedQuery.includes('זול')) {
