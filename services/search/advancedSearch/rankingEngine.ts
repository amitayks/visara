...
export class RankingEngine {
  ...
  private calculateScoringFactors(
    ...
    if (docVector && queryVector) {
      factors.semanticSimilarity = this.cosineSimilarity(docVector, queryVector);
    }
  ...
  private calculateDateRelevance(doc: Document, query: ParsedQuery): number {
    ...
    if (query.temporal.startDate && query.temporal.endDate) {
      if (docDate >= query.temporal.startDate && docDate <= query.temporal.endDate) {
        // Within range - calculate position in range for scoring
        const rangeSize = this.daysBetween(query.temporal.startDate, query.temporal.endDate);
  ...
  private calculatePhoneticScore(vendor: string, queryVendors: string[]): number {
    if (!vendor) return 0;
  ...
  private calculateVendorScore(vendor: string, queryVendors: string[]): number {
    ...
    for (const queryVendor of queryVendors) {
      ...
      const distance = this.levenshteinDistance(normalizedVendor, normalizedQuery);
      const maxLength = Math.max(normalizedVendor.length, normalizedQuery.length);
  ...
  private calculateAmountScore(docAmount: number, amountFilter: any): number {
    ...
    switch (operator) {
      ...
      case 'between':
        if (docAmount >= value && docAmount <= (amountFilter.maxValue || value)) {
  ...
  boostRecent(documents: ScoredDocument[], boostFactor: number = 1.2): ScoredDocument[] {
    const now = new Date();
