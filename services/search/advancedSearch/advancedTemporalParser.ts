import { TemporalExpression, DocumentType, TEMPORAL_KEYWORDS } from './searchTypes';
...
export class AdvancedTemporalParser {
  ...
  private parseCountExpression(text: string): TemporalExpression | null {
    ...
    for (const pattern of countPatterns) {
      ...
      if (match) {
        ...
        if (timeUnit) {
          const { startDate, endDate } = this.calculateDateRange(count, timeUnit, 'past');
          return {
            type: 'relative',
            startDate,
            endDate,
  ...
  private parseRelativeExpression(text: string): TemporalExpression | null {
    ...
    for (const pattern of relativePatterns) {
      ...
      if (match) {
        ...
        if (unit) {
          ...
          const { startDate, endDate } = this.calculateDateRange(count, unit, direction);
    ...
    if (text.includes('this month') || text.includes('החודש')) {
      ...
      const endDate = new Date(this.today.getFullYear(), this.today.getMonth() + 1, 0);
      return {
        type: 'relative',
        startDate,
        endDate,
  ...
  private parseQuarterExpression(text: string): TemporalExpression | null {
    ...
    for (const pattern of quarterPatterns) {
      ...
      if (match) {
        ...
        const year = match[2] ? parseInt(match[2], 10) : this.today.getFullYear();
    ...
    if (text.includes('last quarter') || text.includes('הרבעון האחרון')) {
      ...
      const year = currentQuarter === 1 ? this.today.getFullYear() - 1 : this.today.getFullYear();
  ...
  private extractDocumentType(text: string): DocumentType | null {
    const documentTypes: DocumentType[] = [
      'receipt', 'invoice', 'id', 'form', 'letter', 
      'contract', 'tax', 'medical', 'insurance', 'screenshot'
    ];
  ...
  private extractTimeUnit(text: string): 'day' | 'week' | 'month' | 'quarter' | 'year' | null {
  ...
  private getQuarterRange(quarter: number, year: number): { startDate: Date; endDate: Date } {
  ...
  private getSeasonRange(season: string, year: number): { startDate: Date; endDate: Date } {
    let startDate: Date;
    let endDate: Date;
  ...
  private extractQuarterNumber(text: string): number | null {
    const quarterMap: Record<string, number> = {
      '1': 1, 'first': 1, '1st': 1, 'ראשון': 1,
      '2': 2, 'second': 2, '2nd': 2, 'שני': 2,
      '3': 3, 'third': 3, '3rd': 3, 'שלישי': 3,
      '4': 4, 'fourth': 4, '4th': 4, 'רביעי': 4,
    };
  ...
  combineTemporalExpressions(
    ...
    const startDate = expr1.startDate && expr2.startDate
      ? new Date(Math.max(expr1.startDate.getTime(), expr2.startDate.getTime()))
      : expr1.startDate || expr2.startDate;

    const endDate = expr1.endDate && expr2.endDate
