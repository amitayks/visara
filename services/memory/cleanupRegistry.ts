...
export class CleanupRegistry {
  ...
  register(
    ...
    if (this.executedTasks.has(id)) {
      console.warn(`[CleanupRegistry] Task ${id} already executed, skipping registration`);
      return;
    }
    ...
    console.log(`[CleanupRegistry] Registered cleanup task: ${id} (${priority})`);
  }
    ...
    this.register(
      ...
      async () => {
        try {
          ...
          console.warn(`[CleanupRegistry] Failed to delete file ${path}:`, error);
        }
      },
  ...
  async cleanupAll(): Promise<void> {
    ...
    console.log(`[CleanupRegistry] Starting cleanup of ${this.cleanupTasks.size} tasks`);
    ...
    await Promise.all(
      otherTasks.map(task => this.cleanup(task.id))
    );

    console.log(`[CleanupRegistry] Cleanup completed. Executed: ${this.executedTasks.size} tasks`);
  }
...
export class TempFileTracker {
  ...
  async cleanupAll(): Promise<void> {
    ...
    console.log(`[TempFileTracker] Cleaning ${this.files.size} files from ${this.source}`);
