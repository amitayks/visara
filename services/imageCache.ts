	...
	private async cacheContentUri(
		...
		try {
			...
			try {
				...
				console.log(
					"Content URI caching failed, using original URI:",
					copyError,
				);
	...
	private async trimCache() {
		try {
			...
			let currentSize = entries.reduce(
				(total, [_, item]) => total + (item.size || 0),
				0,
			);
			...
			console.log(
				`Cache trimmed, new size: ${(currentSize / 1024 / 1024).toFixed(2)}MB`,
			);
		} catch (error) {
