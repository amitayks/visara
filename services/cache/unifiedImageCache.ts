...
export class UnifiedImageCache {
  ...
  private constructor(maxSizeMB: number = 50) {
    ...
    console.log(`[UnifiedImageCache] Initialized with max size: ${formatBytes(this.maxSize)}`);
  }
  ...
  private registerMemoryPressureHandler(): void {
    memoryManager.onMemoryPressure(async () => {
      console.log('[UnifiedImageCache] Memory pressure detected, reducing cache');
  ...
  async get(key: string): Promise<string | null> {
    ...
    try {
      ...
      console.warn(`[UnifiedImageCache] Error checking cached file: ${key}`, error);
      this.remove(key);
      this.misses++;
      return null;
    }
  ...
  async set(
    ...
    if (this.totalSize > this.CRITICAL_SIZE) {
      console.warn('[UnifiedImageCache] Cache at critical size, emergency cleanup');
    ...
    if (size > this.maxSize * 0.5) {
      console.warn(`[UnifiedImageCache] Item too large (${formatBytes(size)}), not caching`);
      return;
    }
  ...
  async remove(key: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`[UnifiedImageCache] Failed to delete cached file: ${entry.uri}`);
    }

    this.totalSize -= entry.size;
    this.cache.delete(key);
    
    console.log(`[UnifiedImageCache] Removed ${key}, freed ${formatBytes(entry.size)}`);
  }
  ...
  async cleanExpired(): Promise<number> {
    ...
    if (expired.length > 0) {
      console.log(`[UnifiedImageCache] Cleaned ${expired.length} expired entries`);
    }
  ...
  async migrateFromOldCache(oldCachePath: string): Promise<void> {
    try {
      ...
      console.log(`[UnifiedImageCache] Migrated ${migrated} files from old cache`);
    } catch (error) {
