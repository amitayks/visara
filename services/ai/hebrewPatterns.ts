import { HebrewMetadata } from './ocrTypes';

export class HebrewPatterns {
  static readonly CURRENCY = {
    NIS_SYMBOL: /₪/g,
    NIS_TEXT: /ש"ח|שקל|שקלים|NIS/gi,
    AMOUNT_WITH_CURRENCY: /(?:₪|ש"ח|NIS)\s*(\d{1,3}(?:[,.\s]\d{3})*(?:[.,]\d{2})?)|(\d{1,3}(?:[,.\s]\d{3})*(?:[.,]\d{2})?)\s*(?:₪|ש"ח|NIS)/gi,
  };
  ...
  static readonly BUSINESS = {
    VAT_NUMBER: /מספר עוסק[\s:]*(\d{9})|ע\.מ\.[\s:]*(\d{9})|עוסק מורשה[\s:]*(\d{9})/gi,
    COMPANY_NUMBER: /ח\.פ\.[\s:]*(\d{9})|חברה[\s:]*(\d{9})|מספר חברה[\s:]*(\d{9})/gi,
  ...
  static readonly DATE = {
    ...
    // Hebrew month names
    HEBREW_MONTHS: /ינואר|פברואר|מרץ|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר/gi,
    // Date with Hebrew month: 15 באפריל 2024
    HEBREW_DATE: /(\d{1,2})\s*ב?(ינואר|פברואר|מרץ|אפריל|מאי|יוני|יולי|אוגוסט|ספטמבר|אוקטובר|נובמבר|דצמבר)\s*(\d{2,4})/gi,
  };
  ...
  static extractHebrewMetadata(text: string): HebrewMetadata {
    ...
    const currencyMatches = text.matchAll(this.CURRENCY.AMOUNT_WITH_CURRENCY);
    ...
    const mobilePhones = text.match(this.PHONE.MOBILE) || [];
    const landlines = text.match(this.PHONE.LANDLINE) || [];
    metadata.phones = [...new Set([...mobilePhones, ...landlines])];

    // Extract VAT numbers
    const vatMatches = text.matchAll(this.BUSINESS.VAT_NUMBER);
    ...
    const companyMatches = text.matchAll(this.BUSINESS.COMPANY_NUMBER);
    ...
    // Extract dates
    const standardDates = text.matchAll(this.DATE.STANDARD);
    ...
    const hebrewDates = text.matchAll(this.DATE.HEBREW_DATE);
