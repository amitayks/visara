...
export class ReceiptExtractor implements DocumentExtractor<ReceiptData> {
	...
	private async extractTotals(
		text: string,
		entities: Entity[],
	): Promise<ReceiptData["totals"]> {
		let subtotal = 0;
		let tax = 0;
		let tip = 0;
		let total = 0;
	...
	private extractMetadata(
		...
		const cashierMatch = text.match(
			/(?:cashier|served by|operator)[\s:]*([A-Za-z\s]+)/i,
		);
		if (cashierMatch) {
		...
		const storeMatch = text.match(
			/(?:store|branch|location)[\s#:]*([A-Za-z0-9\s]+)/i,
		);
		if (storeMatch) {
	...
	private categorizeItem(description: string): string | undefined {
		const categories = new Map([
			[
				"food",
				["coffee", "sandwich", "burger", "pizza", "salad", "soup", "meal"],
			],
			[
				"beverage",
				["drink", "soda", "juice", "water", "tea", "latte", "cappuccino"],
			],
			["retail", ["shirt", "pants", "shoes", "jacket", "accessories"]],
			[
				"grocery",
				["milk", "bread", "eggs", "cheese", "meat", "vegetables", "fruit"],
			],
	...
	async validate(data: ReceiptData): Promise<ValidationResult> {
		...
		const calculatedTotal =
			data.totals.subtotal + data.totals.tax + (data.totals.tip || 0);
		if (
