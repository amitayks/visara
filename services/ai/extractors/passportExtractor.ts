...
export class PassportExtractor implements DocumentExtractor<PassportData> {
	...
	private extractVisualData(text: string, entities: Entity[]): PersonalInfo {
		...
		const nationalityPattern =
			/(?:nationality|citizenship)[\s:]*([A-Za-z\s]+)/gi;
		...
		for (const block of blocks) {
			...
			if (stampKeywords.some((keyword) => blockText.includes(keyword))) {
				...
				if (blockText.includes("entry") || blockText.includes("arrival")) {
					...
					blockText.includes("exit") ||
					blockText.includes("departure")
				) {
					stampType = "exit";
				}
	...
	private checkValidity(
		...
		if (
			mrzData.documentNumber === "UNKNOWN" ||
			mrzData.documentNumber.length < 6
		) {
		...
		if (visualData.firstName && visualData.lastName) {
			const visualFullName =
				`${visualData.firstName} ${visualData.lastName}`.toUpperCase();
			const mrzFullName =
				`${mrzData.givenNames} ${mrzData.surname}`.toUpperCase();

			if (
				visualFullName !== mrzFullName &&
