...
export class DocumentExtractorFactory {
	...
	async initialize(): Promise<void> {
		...
		const initPromises = Array.from(this.extractors.values()).map(
			async (extractor) => {
				try {
					...
					console.warn(
						`✗ Failed to initialize ${extractor.constructor.name}:`,
						error,
					);
				}
			},
	...
	getExtractor(documentType: DocumentType): DocumentExtractor<StructuredData> {
		...
		console.log(
			`No specific extractor for ${documentType}, using generic extractor`,
		);
	...
	getBestExtractor(contextualResult: ContextualResult): {
		extractor: DocumentExtractor<StructuredData>;
		confidence: number;
	} {
	...
	private estimateExtractorFit(
		...
		switch (documentType) {
			...
			case DocumentType.ID_CARD:
				if (text.includes("id") || text.includes("identification"))
					score += 0.3;
	...
	async extractWithMultipleExtractors(
		...
		if (contextualResult.confidence < 0.8) {
			...
			for (const altType of alternatives.slice(0, 2)) {
				// Max 2 alternatives
				try {
		...
		const bestAlternative = alternativeResults.reduce<{
			extractor: string;
			result: StructuredData;
			confidence: number;
		} | null>(
