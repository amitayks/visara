...
export class GenericDocumentExtractor
	implements DocumentExtractor<GenericDocumentData>
{
	...
	private extractKeyValuePairs(text: string): Array<{
		...
		for (const line of lines) {
			...
			if (words.length >= 2 && words.length <= 6) {
				const possibleKey = words
					.slice(0, Math.ceil(words.length / 2))
					.join(" ");
				const possibleValue = words
		...
		const uniquePairs = pairs.filter(
			(pair, index, array) =>
				array.findIndex(
					(p) => p.key.toLowerCase() === pair.key.toLowerCase(),
				) === index,
		);
	...
	private looksLikeAddress(text: string): boolean {
		return /\d+\s+[A-Za-z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Boulevard|Blvd|Lane|Ln|Drive|Dr)/i.test(
			text,
		);
	}
	...
	private looksLikeKeyValuePair(key: string, value: string): boolean {
		...
		const hasCommonKeyPattern = commonKeyPatterns.some((pattern) =>
			pattern.test(key),
		);
	...
	async validate(data: GenericDocumentData): Promise<ValidationResult> {
		...
		if (data.keyValuePairs.length === 0) {
			suggestions.push(
				"No structured key-value pairs found - document may be unstructured text",
			);
		} else {
			// Boost confidence for well-structured documents
			const highConfidencePairs = data.keyValuePairs.filter(
				(pair) => pair.confidence > 0.7,
			);
		...
		if (!data.title) {
			suggestions.push(
				"Consider adding a title or header for better organization",
			);
		}
	...
	async extractTables(text: string): Promise<
		Array<{
			headers: string[];
			rows: string[][];
			confidence: number;
	...
	async extractSections(text: string): Promise<
		Array<{
			title: string;
			content: string;
			startLine: number;
	...
	private looksLikeSectionHeader(
		line: string,
		allLines: string[],
		index: number,
	): boolean {
		...
		const nextLine =
			index + 1 < allLines.length ? allLines[index + 1].trim() : "";
		if (
			nextLine.length === 0 ||
			this.looksLikeSectionHeader(nextLine, allLines, index + 1)
		) {
