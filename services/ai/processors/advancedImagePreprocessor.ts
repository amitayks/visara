...
export class AdvancedImagePreprocessor {
  ...
  private readonly CACHE_DIR = `${RNFS.CachesDirectoryPath}/hybrid_preprocessed/`;
  ...
  async processForOCR(
    ...
    try {
      ...
      if (opts.autoRotate && Math.abs(analysis.orientation) > 1) {
        processedUri = await this.autoRotate(processedUri, analysis.orientation);
      ...
      if (opts.enhanceContrast && (analysis.brightness < 0.3 || analysis.brightness > 0.8)) {
      ...
      if (opts.targetLanguages.includes('he') || opts.targetLanguages.includes('ar')) {
      ...
      console.log(`Image preprocessing completed in ${processedImage.processingTime}ms`);
  ...
  private async autoRotate(imageUri: string, orientation: number): Promise<string> {
    try {
  ...
  private async enhanceContrast(imageUri: string, analysis: ImageAnalysis): Promise<string> {
    // For now, return original URI
    // In a full implementation, you'd apply contrast enhancement algorithms
    console.log(`Would enhance contrast for brightness: ${analysis.brightness}`);
    return imageUri;
  }
  ...
  private async manageCacheSize(): Promise<void> {
    ...
    for (const [key, processedImage] of toRemove) {
      try {
        ...
        console.warn(`Failed to remove cached file: ${processedImage.uri}`, error);
      }
    }
  }
