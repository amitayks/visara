...
export class MultiEngineOCR {
	...
	async initialize(): Promise<void> {
		...
		const initPromises = Array.from(this.engines.values()).map(
			async (engine) => {
				try {
	...
	async extractText(imageUri: string): Promise<OCRResult> {
		...
		try {
			...
			console.log(
				`Multi-engine OCR completed in ${finalResult.processingTime}ms`,
			);
	...
	private async runEngines(
		...
		const enginePromises = availableEngines.map(async (engine) => {
			...
			try {
				...
				if (!result || typeof result !== "object") {
					console.warn(
						`Engine ${engine.name} returned invalid result:`,
						result,
					);
					return null;
				}
				...
				const validatedResult: OCRResult = {
					...
					processingTime:
						typeof result.processingTime === "number"
							...
							: 0,
					language: [
						typeof result.language === "string" ? result.language : "en",
					],
		...
		return engineResults.filter(
			(result): result is OCREngineResult => result !== null,
		) as OCREngineResult[];
	}

	private async selectBestResult(
		engineResults: OCREngineResult[],
	): Promise<OCRResult> {
		...
		const sortedResults = engineResults.sort(
	...
	private async fallbackSingleEngine(imageUri: string): Promise<OCRResult> {
		...
		try {
			...
			if (!result || typeof result !== "object") {
				throw new Error(
					`Fallback engine ${engine.name} returned invalid result`,
				);
			}
			...
			throw new Error(
				`All OCR engines failed. Last error: ${error instanceof Error ? error.message : String(error)}`,
			);
		}
	}
	...
	getAvailableEngines(): LocalOCREngine[] {
		const engines = Array.from(this.engines.values()).filter(
			(engine) => engine.isInitialized && engine.isInitialized(),
		);
