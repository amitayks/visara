...
export class QualityAssurance {
	...
	private async runAllChecks(input: QualityAssessmentInput): Promise<Check[]> {
		...
		checks.push(
			...this.checkStructuredData(
				input.structuredData,
				input.contextualResult.documentType,
			),
		);
	...
	private checkOCRQuality(ocrResult: OCRResult): Check[] {
		...
		checks.push({
			...
			suggestion:
				textLength <= 10 ? "Check image quality and OCR settings" : undefined,
		});

		// Check block quality
		const avgBlockConfidence =
			ocrResult.blocks.reduce((sum, block) => sum + block.confidence, 0) /
	...
	private checkContextUnderstanding(
		contextualResult: ContextualResult,
	): Check[] {
		const checks: Check[] = [];
		...
		checks.push({
			...
			message:
				contextualResult.confidence >= 0.6
	...
	private checkReceiptData(data: ReceiptData): Check[] {
		...
		if (data.items && data.totals) {
			...
			checks.push({
				...
				confidence:
					calculationDiff < 0.1 ? 1 : calculationDiff < 1.0 ? 0.7 : 0.3,
				message:
					calculationDiff < 1.0
	...
	private checkIDData(data: IDData): Check[] {
		...
		checks.push({
			...
			passed:
				data.personalInfo.firstName.length > 0 &&
				data.personalInfo.lastName.length > 0,
			confidence:
				data.personalInfo.firstName.length > 0 &&
				data.personalInfo.lastName.length > 0
					? 0.9
		...
		checks.push({
			...
			message:
				data.documentInfo.documentNumber.length > 0
		...
		const now = new Date();
		const isExpired =
			data.documentInfo.expiryDate && data.documentInfo.expiryDate < now;
		checks.push({
			...
			suggestion: isExpired ? "Verify expiry date on the document" : undefined,
		});
	...
	private checkPassportData(data: PassportData): Check[] {
		...
		const visualName =
			`${data.visualData.firstName} ${data.visualData.lastName}`.trim();
		const namesMatch =
			mrzName.toLowerCase().includes(visualName.toLowerCase()) ||
		...
		checks.push({
			...
			message:
				namesMatch || visualName.length === 0
	...
	private checkConsistencyAcrossLayers(input: QualityAssessmentInput): Check[] {
		...
		const contextDirection =
			input.contextualResult.context.layout.textDirection;
	...
	private calculateCompleteness(
		...
		score +=
			this.calculateStructuredDataCompleteness(
				structuredData,
				contextualResult.documentType,
			) * 0.2;

		return Math.min(1, score);
	}

	private calculateStructuredDataCompleteness(
		data: StructuredData,
		documentType: DocumentType,
	): number {
		switch (documentType) {
			...
			case DocumentType.DRIVERS_LICENSE:
				...
				if (
					!idData ||
					!("personalInfo" in idData) ||
					!("documentInfo" in idData)
				)
					return 0;
				return (
	...
	private calculateConsistency(
		...
		score -= this.checkDocumentSpecificConsistency(
			structuredData,
			contextualResult.documentType,
		);
	...
	private checkDocumentSpecificConsistency(
		data: StructuredData,
		documentType: DocumentType,
	): number {
		let penalty = 0;
		...
		switch (documentType) {
			case DocumentType.RECEIPT:
				...
				if (
					receiptData.totals.subtotal > 0 &&
					Math.abs(itemsTotal - receiptData.totals.subtotal) > 1.0
				) {
					penalty += 0.2;
				}
				break;
