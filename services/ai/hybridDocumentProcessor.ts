...
export class HybridDocumentProcessor
	implements HybridDocumentProcessorInterface
{
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			console.log(
				`Hybrid Document Processor initialized in ${Date.now() - startTime}ms`,
			);
		} catch (error) {
			console.error("Failed to initialize Hybrid Document Processor:", error);
			throw new Error(
				`Initialization failed: ${error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)}`,
			);
		}
	}
	...
	async processDocument(
		...
		try {
			...
			processingSteps.push(
				`OCR completed with ${ocrResult.confidence.toFixed(2)} confidence`,
			);
			...
			if (opts.enableContextUnderstanding) {
				...
				processingSteps.push(
					`Context analysis completed: ${contextualResult.documentType}`,
				);
			} else {
			...
			if (opts.enableStructuredExtraction) {
				...
				structuredData =
					await this.createGenericStructuredData(contextualResult);
			...
			if (totalProcessingTime > opts.maxProcessingTime) {
				warnings.push(
					`Processing exceeded time limit: ${totalProcessingTime} > ${opts.maxProcessingTime}ms`,
				);
			}

			// Calculate image hash
			const preprocessedImage =
				await this.imagePreprocessor.preprocessImage(imageUri);
			...
			console.log(
				`Document type: ${contextualResult.documentType} (${contextualResult.confidence})`,
			);
	...
	async extractStructuredData(
		...
		try {
			console.log(
				`Extracting structured data for ${contextualResult.documentType}...`,
			);
	...
	private async createBasicContextualResult(
		ocrResult: OCRResult,
	...
	private async createGenericStructuredData(
		contextualResult: ContextualResult,
	): Promise<StructuredData> {
		return {
