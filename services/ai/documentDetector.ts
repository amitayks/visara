...
export class DocumentDetector {
	...
	private async analyzeForText(imageUri: string) {
		try {
			...
			const hasText = textKeywords.some((keyword) =>
				filename.includes(keyword),
			);
			const textDensity = hasText
				? Math.random() * 0.5 + 0.3
				: Math.random() * 0.2; // 0.3-0.8 for docs, 0-0.2 for photos
	...
	private calculateConfidence(basicAnalysis: any, textAnalysis: any): number {
		...
		if (
			dominantColors.includes("#FFFFFF") &&
			dominantColors.includes("#000000")
		) {
			confidence += 0.2;
		}
