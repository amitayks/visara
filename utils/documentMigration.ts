      ...
      for (const doc of allDocuments) {
        try {
          ...
          if (doc.imageUri && doc.imageUri.includes('cache')) {
            console.log(`Checking document ${doc.id} with URI: ${doc.imageUri}`);
            
            // Check if the temporary file still exists
            const tempFileInfo = await FileSystem.getInfoAsync(doc.imageUri);
            
            if (tempFileInfo.exists) {
              console.log('Temporary file still exists, migrating...');
              
              // Copy to permanent storage
              const permanentUri = await imageStorage.copyImageToPermanentStorage(
                doc.imageUri,
                doc.imageHash
              );
              ...
              console.log(`Temporary file no longer exists for document ${doc.id}`);
              failed++;
            }
          } else if (doc.imageUri && doc.imageUri.includes('visara_documents')) {
            console.log(`Document ${doc.id} already using permanent storage`);
            skipped++;
          } else {
            console.log(`Document ${doc.id} has unknown URI format: ${doc.imageUri}`);
            skipped++;
          }
        } catch (error) {
    ...
    try {
      ...
      for (const doc of allDocuments) {
        if (doc.imageUri) {
          ...
          if (!fileInfo.exists) {
            missingCount++;
            console.log(`Missing image for document ${doc.id}: ${doc.imageUri}`);
          }
        }
      }
...
export const migrateDocuments = () => DocumentMigration.migrateExistingDocuments();
export const checkDocumentStorage = () => DocumentMigration.checkDocumentStorage();