  ...
  const gestureHandler = useAnimatedGestureHandler({
    ...
    onActive: (event, context) => {
      ...
      if (newTranslateY >= 0) {
        ...
        opacity.value = Math.max(0.3, 1 - newTranslateY / (SCREEN_HEIGHT * 0.5));
      }
    },
    onEnd: (event) => {
      const shouldDismiss = event.translationY > SWIPE_THRESHOLD || event.velocityY > 500;
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert("Error", "Failed to delete document: " + (error instanceof Error ? error.message : String(error)));
            } finally {
              setIsDeleting(false);
            }
          },
            ...
            <View style={styles.detailsSection}>
              ...
              {(document.totalAmount !== null && document.totalAmount !== undefined && document.totalAmount > 0) && (
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.detailValue}>
                    {document.currency || "$"}{document.totalAmount.toFixed(2)}
                  </Text>
                </View>
              )}
